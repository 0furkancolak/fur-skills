...
3. Grade the output using `src/references/output-rubrics.md`.
4. Record scores and notes.

### Automated (deterministic checks only)

```bash
bun src/evals/fur-do/graders/deterministic.ts src/skills/fur-do/SKILL.md <output.md>
```
...
|---|---|---|
| Deterministic | Exact checks: frontmatter, headings, YAML | `graders/deterministic.ts` |
| Rubric | Score depth, quality, evidence, risk | Manual using `src/references/output-rubrics.md` |
